<div>
    @if ($submitted)
        <div class="space-y-4 text-center">
            ...
            <a href="{{ route('welcome') }}" class="text-sm font-medium text-orange-500 hover:text-orange-400">
                Return to homepage
            </a>
        </div>
    @else
            ...
            <div class="flex flex-col space-y-8">
                <div class="flex justify-evenly">
                    @foreach ($steps as $index => $_)
                        <div class="rounded-3xl px-5 py-3 {{ $index === $step ? 'bg-orange-600 text-white' : 'bg-gray-800 text-neutral-400' }}">
                            {{ $index + 1 }}
                        </div>
                    @endforeach
                </div>
